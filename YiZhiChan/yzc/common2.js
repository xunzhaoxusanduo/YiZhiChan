...
lansum.slide.s1 = function () {
    ...
    this.init = function (isBindEvent) {
        ...
        if (this.isBlur) {
            ...
            for (var i = $imgs.length - 1; i >= 0; i--) {
                ...
                this.$selContainer.prepend('<img id="img' + i + '" class="bg img2" />')
            }
    ...
    this.funMove = function (obj, isAnimate) {
        ...
        if (obj.isBlur) {
            ...
            bgList.fadeOut(500);
            bgCurrent.fadeIn(500);

            var fun = function (iii, src) {
                return function () {
                    ...
                    var img = new Image();
                    img.onload = function () {
                        bctx.drawImage(img, 0, 0, img.width, img.height, 0, 0, buff.width, buff.height);
                        StackBlur.canvasRGB(buff, 0, 0, buff.width, buff.height, 50);
                        $('#img' + img.alt)
                            .attr('data-loaded', '1')
                            .attr('src', buff.toDataURL());

                        console.info("img:" + img.alt);
                    };
                    img.alt = iii;
                    img.src = src;
                }
            }
            var imgsrc = obj.$selItem.find('img').eq(obj.currentIndex - 1).attr('src');
            var ex = fun(obj.currentIndex - 1, imgsrc);
            var isLoad = $('#img' + (obj.currentIndex - 1)).attr('data-loaded');
            if (isLoad==undefined) {
                ex();
            }
